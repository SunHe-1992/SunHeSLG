...
public class ThirdPersonOrbitCamBasic : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Mouse:
        bool isMobile = GamePlayManager.Instance.GetIsMobile();
        ...
        if (isMobile == true)
        {
            if (Input.touchCount > 0)
            {
                ...
                for (int i = 0; i < Input.touchCount; i++)
                {
                    ...
                    var touchI = Input.GetTouch(i);
                    Vector2 touchPct = new Vector2(
   (touchI.position.x / Screen.width) * 100,
   (touchI.position.y / Screen.height) * 100);
                    //only accept touch on the right side of screen
                    if (touchPct.y < 60 && touchPct.x < 35)
                    {
                    }
                    else
                    {
                ...
                if (foundTouch)
                {
                    Vector2 posDelta = (touch.position - previousPosition);
                    angleH += Mathf.Clamp(posDelta.x, -1, 1) * horizontalAimingSpeed;
                    angleV += Mathf.Clamp(posDelta.y, -1, 1) * verticalAimingSpeed;

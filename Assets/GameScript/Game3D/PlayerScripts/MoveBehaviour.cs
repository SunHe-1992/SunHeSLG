...
public class MoveBehaviour : GenericBehaviour
{
    ...
    {
        // Get jump input.
        if (!jump && IsJump() && behaviourManager.IsCurrentBehaviour(this.behaviourCode) && !behaviourManager.IsOverriding())
        {
            jump = true;
        }
    }

    public static bool clickJump = false;
    bool IsJump()
    {
        if (GamePlayManager.Instance.GetIsMobile())
        {
            return clickJump;
        }
        else
